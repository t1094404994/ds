...
import {GridConfig, VertexType, getGridPt, getGridIndexByGridPt} from './graph'
...
//开始节点表单数据
export type FromStartData=FromStartType&FromBaseData&FromPromoterData&FromCcData
...
//一般节点表单数据
export type FromNodeData=FromNodeType&FromNodeBaseData&FromApproverData&FromCcData
...
//顶点审批流数据
export type VertexBPMNData=BPMNStartData|BPMNNodeData|BPMNBranchData|BPMNEndData
...
//创建节点数据
export const createVertexBPMNData = (type:VertexType, id:string, text:string):VertexBPMNData => {
  switch (type) {
  ...
  case VertexType.NODE:
    return {type, id, name:text, canRevoke:0, approvalMethod:0, allowSigning:0, call:0}
  }
}
//创建边数据
export const createEdgeBPMNData = (vId:string, jId:string, text:string, disable?:boolean):EdgeBPMNData => {
  const id = createBPNMId('-E')
  return {id, priority:1, sourceRef:vId, targetRef:jId, conditionName:text, nodeCondition:1, disable}
}
...
//获取会签/或签信息 后端用
const getMultiInstanceLoopCharacteristics = (bool:boolean):MultiInstanceLoopCharacteristics => {
  return  {
    '@_isSequential':'false',
    '@_flowable:collection':'flwUserList',
    '@_flowable:elementVariable':'approval',
    loopCardinality:'${loopCardinality}',
    completionCondition:bool ? '${nrOfCompletedInstances == 1}' : '${nrOfCompletedInstances/nrOfInstances >= 1}'
  }
}
...
const createXMLOrgStr = (data?:Dictionary<Array<string>>):string|undefined => {
  let str = ''
  if (data) {
...
const createXMLStart = (start:BPMNStartData) => {
  const {id, name, documentation, orgPromoters, userPromoters, postPromoters, notifiers} = start
...
const getXmlNodeDataType = (orgApprovalers?:Dictionary<Array<string>>, userApprovalers?:Array<string>, postApprovalers?:Array<string>):string => {
...
const createXMLNode = (node:BPMNNodeData):XMLNode => {
  const {id, name, documentation, orgApprovalers, userApprovalers, postApprovalers, notifiers, canRevoke, approvalMethod, allowSigning, call, callAddress} = node
  const xmlNode:XMLNode = {
    '@_id':id,
    '@_name':name,
    'documentation':documentation,
    '@_flowable:assignee':'${approval}',
    '@_flowable:dataType':getXmlNodeDataType(orgApprovalers, userApprovalers, postApprovalers),
    ...
    multiInstanceLoopCharacteristics:getMultiInstanceLoopCharacteristics(approvalMethod ? true : false)
  }
  return xmlNode
}
//创建分支节点数据
...
const createXmlEdge = (edge:EdgeBPMNData):XMLEdge => {
  const {id, sourceRef, targetRef, conditionName, CDATA, nodeCondition, priority} = edge
  const xmlEdge:XMLEdge = {
    '@_id':id,
    '@_name':conditionName,
    '@_sourceRef':sourceRef,
    '@_targetRef':targetRef,
    extensionElements:{'flowable:priority':priority + '', 'flowable:nodeCondition':nodeCondition ? 'true' : 'false'},
    conditionExpression:CDATA ? {'@_xsi:type':'tFormalExpression', '#text':`\<![CDATA[${CDATA}]]\>`} : undefined
  }
  return xmlEdge
}
//创建图形节点
const createXMLPlane = (vertexId:string, gridIndex:number, gridConfig:GridConfig):XMLPlane|false => {
  const pt = getGridPt(gridIndex, gridConfig)
  const xmlPlane:XMLPlane = {
    '@_id':vertexId + '_plane',
    '@_bpmnElement':vertexId,
    'omgdc:Bounds':{'@_height':'0', '@_width':'0', '@_x':pt.x + '', '@_y':pt.y + ''}
  }
  return xmlPlane
}

//根据流程图和网格配置,创建XML数据
export const createXMLData = (id:string, name:string|false, flowChart:FlowChart, gridConfig:GridConfig):XMLData|false => {
  ...
  for (let i = 0; i < vertexNum; i++) {
    ...
    //顶点图形信息
    const plane = createXMLPlane(vertextData.data.id as string, vertextData.gridIndex, gridConfig)
    if (!plane) continue
    planes.push(plane)
  }
  if (!startEvent || !extensionElements || !endEvent || !userTask.length || !sequenceFlow.length) return false
  const xmlProcess:XMLProcess = {'@_id':id, '@_name':name ? name : id, '@_isExecutable':'true', extensionElements, startEvent, userTask, exclusiveGateway, endEvent, sequenceFlow}
...
//获取XML的机构数据 也有父级id
//orgId1:[structureId1,structureId2,.....] , orgId2:[structureId1,structureId2,.....]
export const getOrgData = (data:string|undefined):Dictionary<Array<string>> => {
  if (!data) return {}
...
export const getStartData = (xmlStart:XMLStart, extension:XMLStartExtension, plane:XMLPlane, gridConfig:GridConfig):GetNodeData => {
...
export const getNodeData = (xmlNode:XMLNode, plane:XMLPlane, gridConfig:GridConfig):GetNodeData => {
...
//根据XML分支节点,获取插入节点的数据
export const getBranchData = (xmlBranch:XMLBranch, plane:XMLPlane, gridConfig:GridConfig):GetNodeData => {
...
//根据XML结束节点，获取插入节点的数据
export const getEndData = (xmlEnd:XMLEnd, plane:XMLPlane, gridConfig:GridConfig):GetNodeData => {
...
export const getEdgeData = (i:number, j:number, xmlEdge:XMLEdge) => {
  const bpmnData:EdgeBPMNData = {
    ...
    nodeCondition:xmlEdge.extensionElements['flowable:nodeCondition'] === 'true' ? 1 : 0,
    sourceRef:xmlEdge['@_sourceRef'],
    targetRef:xmlEdge['@_targetRef'],
    conditionName:xmlEdge['@_name'] ? xmlEdge['@_name'] : '设置条件',
    CDATA:xmlEdge.conditionExpression && xmlEdge.conditionExpression['#text'] ? getConditionExpression(xmlEdge.conditionExpression['#text']) : undefined
  }
  return {i, j, bpmnData}
}
...
export const getNodeEdgeData = (xmlData:XMLData, gridConfig:GridConfig) => {
  ...
  const planes = xmlData.definitions['bpmndi:BPMNDiagram']['bpmndi:BPMNPlane']['bpmndi:BPMNShape']
  ...
  //开始节点
  const startData = getStartData(process.startEvent, process.extensionElements, planeMap[process.startEvent['@_id']], gridConfig)
  nodeArr.push(startData)
  //一般节点
  const userTasks:Array<XMLNode> = Array.isArray(process.userTask) ? process.userTask : [process.userTask]
  userTasks.forEach((node) => {
    const nodeData = getNodeData(node, planeMap[node['@_id']], gridConfig)
    nodeArr.push(nodeData)
  })
  //分支节点
  const exclusiveGateways:Array<XMLBranch>|undefined = Array.isArray(process.exclusiveGateway) ? process.exclusiveGateway : [process.exclusiveGateway]
  if (exclusiveGateways) {
    exclusiveGateways.forEach((branch) => {
      if (branch) {
        const branchData = getBranchData(branch, planeMap[branch['@_id']], gridConfig)
        nodeArr.push(branchData)
      }
    })
  }
  //结束节点
  const endData = getEndData(process.endEvent, planeMap[process.endEvent['@_id']], gridConfig)
...
export const verificationData = (data:XMLData) => {
  ...
  const {startEvent, endEvent, userTask, exclusiveGateway, sequenceFlow} = process
  ...
  for (let i = 0, l = taskArr.length; i < l; i++) {
    ...
    if (!task['@_flowable:canRevoke'] || !task['@_flowable:allowSigning'] || !task['@_flowable:approvalMethod'] || !task['@_flowable:call']) return false
    ...
    if (task['@_flowable:call'] === 'true' && (!task['@_flowable:callAddress'] || task['@_flowable:callAddress'].length > 200)) {
      ElMessage.warning(`节点${taskName}回调地址未填或超过200个字符`)
      return false
    }
  }
  if (exclusiveGateway) {
    const exclusiveGatewayArr = Array.isArray(exclusiveGateway) ? exclusiveGateway : [exclusiveGateway]
    for (let i = 0, l = exclusiveGatewayArr.length; i < l; i++) {
      ...
      if (exclusiveGateway.documentation && exclusiveGateway.documentation.length > 200) {
        ElMessage.warning(`分支${exclusiveGatewayName}描述不能超过200个字符`)
        return false
      }
    }
  }
  const sequenceFlowArr = Array.isArray(sequenceFlow) ? sequenceFlow : [sequenceFlow]
  //出边散列
