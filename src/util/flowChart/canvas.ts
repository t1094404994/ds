...
export enum OperationStatusBit{HOVER, DOWN, MOVE, Up}
//图形的绘制位状态 阴影,选中,拖动,拖动虚化
export enum DrawStatusBit{SHADOW, CLICK, DROP, DROPSHADOW}
//图形类型 圆角，矩形，多边形
export enum GraphType{ARC, RECT, POLYGON}
//图层
export enum LayerType{BG, TOP}
//盒子x1 x2 y1 y2
...
//处理填充和描边
const onFillAndStroke = (ctx:CanvasRenderingContext2D, config:BaseConfig, a?:number) => {
...
const addDotted = (ctx:CanvasRenderingContext2D, box:Box, offset:number) => {
  ...
  ctx.strokeRect(box[0] - offset, box[2] - offset, box[1] - box[0] + offset * 2, box[3] - box[2] + offset * 2)
  ctx.restore()
}
...
export const drawArc = (ctx:CanvasRenderingContext2D, config:ArcConfig) => {
  ...
  ctx.arc(config.anchorX, config.anchorY, config.r, config.startAngle, config.endAngle)
  ctx.closePath()
...
//绘制多边形
export const drawPolygon = (ctx:CanvasRenderingContext2D, config:PolygonConfig) => {
  ctx.save()
  ctx.beginPath()
  if (checkBit(config.drawStatus, DrawStatusBit.SHADOW)) {
    addShadow(ctx)
...
const drawText = (ctx:CanvasRenderingContext2D, config:TextConfig) => {
  ...
  const startY = Math.floor(config.anchorY - config.size / 2 - ((lines.length) * lineHeight - lineSpace) / 2)
  lines.forEach((item, index) => {
    ctx.fillText(item, startX, startY + lineHeight * (index + 1), config.maxWidth)
  })
...
const getRectInfor = (anchorX:number, anchorY:number, w:number, h:number) => {
...
const getPolyInfor = (anchorX:number, anchorY:number, w:number, h:number) => {
  const path:Array<Pt> = []
  const wHalf = Math.floor(w / 2)
  const hHalf = Math.floor(h / 2)
  ...
  path.push({x:anchorX + wHalf, y:anchorY})
  path.push({x:anchorX, y:anchorY + hHalf})
  const box :Box = [anchorX - wHalf, anchorX + wHalf, anchorY - hHalf, anchorY + hHalf]
  return {box, path}
}

//创建图形配置
export const createDrawConfig = (graphType:GraphType, pt:Pt, _r?:number, _w?:number, _h?:number, _radius?:number):DrawConfig => {
  const anchorX = pt.x
  const anchorY = pt.y
  ...
  const base = {anchorX, anchorY, strokeStyle, operationStatus, drawStatus, fillStyle}
...
export const updateDrawConfig = (config:DrawConfig) => {
  ...
  if (config.graphType === GraphType.RECT) {
    const {x, y, width, height, box} = getRectInfor(config.anchorX, config.anchorY, config.width, config.height)
    config.x = x
    config.y = y
    config.width = width
    ...
    const box = getArcBox({anchorX:config.anchorX, anchorY:config.anchorY}, config.r)
    config.box = box
  }
}

//创建默认文字配置
export const createTextConfig = (pt:Pt, text:string, maxWidth:number, i?:number, j?:number):TextConfig => {
  const anchorX = pt.x
  const anchorY = pt.y
  ...
  const operationStatus = 0
  const lineSpace = 5
  return {anchorX, anchorY, font, size, fillStyle, operationStatus, lineSpace, maxWidth, text, i, j}
}
...
const getArcPoint = (_box:Box) => {
  const box:Box = [_box[0] - clickOffset, _box[1] + clickOffset, _box[2] - clickOffset, _box[3] + clickOffset]
...
//绘制边
export const drawEdge = (ctx:CanvasRenderingContext2D, edgeConfig:EdgeConfig) => {
  ctx.save()
  ctx.beginPath()
...
export const getEdgeArrowRadian = (linePath:Array<Pt>) => {
  ...
  return Math.atan2(lastPt.y - beforLastPt.y, lastPt.x - beforLastPt.x) + Math.PI / 2
}
...
export const createEdgeConfig = (linePath:Array<Pt>):EdgeConfig|false => {
  ...
  const arrowW = 8
  return {anchorX, anchorY, fillStyle, strokeStyle, linePath, arrowW, arrowRadian}
}
...
export class Layer {
  ...
  protected checkDropMove(status:OperationStatusBit) {
    return checkBit(status, OperationStatusBit.DOWN) && checkBit(status, OperationStatusBit.MOVE)
  }
  //拖动完成
...
export class BgLayer extends Layer {
  ...
  //设置图形和文字配置
  public setConfigs(_graphConfigs:Array<DrawConfig>, _textCongfigs:Array<TextConfig>, _edgeConfigs:Array<EdgeConfig>, _edgeTextConfigs:Array<TextConfig>) {
    if (!this.ctx || !this.canvas) return false
  ...
  private pollingStatus(pt:Pt, operationStatus:OperationStatusBit) {
    ...
    if (this.graphConfigs) {
      this.graphConfigs.forEach((config, i) => {
        ...
        config.operationStatus = setBit(config.operationStatus, operationStatus, isStatus)
        if (changed) {
          drawflag = true
        ...
        if (this.checkDropMove(config.operationStatus)) {
          drawflag = true
          mitt.emit('dragVertexStart', {vertex:i, layer:this._layerType, x:pt.x, y:pt.y})
        }
      })
    }
    if (this.edgeTextConfigs) {
      this.edgeTextConfigs.forEach((config) => {
        if (config.i !== undefined && config.j !== undefined && config.box) {
          ...
          config.operationStatus = setBit(config.operationStatus, operationStatus, isStatus)
          if (this.checkClick(config.operationStatus)) {
            mitt.emit('edgeClick', {layer:this._layerType, edge:{i:config.i, j:config.j}})
          }
        }
      })
    }
    ...
    if (operationStatus === OperationStatusBit.DOWN && this.clickBox) {
      ...
      for (let i = 0, l = arcBoxs.length; i < l; i++) {
        if (checkIn(pt.x, pt.y, arcBoxs[i])) {
          mitt.emit('dragEdgeStart', {vertex:-1, layer:this._layerType, x:pt.x, y:pt.y})
          break
        }
      }
    }
  ...
  private onMouseMove(evt:MouseEvent) {
    ...
    evt.preventDefault()
    this.pollingStatus({x:evt.offsetX, y:evt.offsetY}, OperationStatusBit.HOVER)
    this.pollingStatus({x:evt.offsetX, y:evt.offsetY}, OperationStatusBit.MOVE)
  }

  private onMouseDown(evt:MouseEvent) {
    evt.preventDefault()
    this.pollingStatus({x:evt.offsetX, y:evt.offsetY}, OperationStatusBit.DOWN)
  }

  private onMouseUp(evt:MouseEvent) {
    evt.preventDefault()
    this.pollingStatus({x:evt.offsetX, y:evt.offsetY}, OperationStatusBit.Up)
  }
...
export class TopLayer extends Layer {
  ...
  public getCurrentConfigPt():Pt|false {
    if (this.currentVertexConfig) {
      return {x:this.currentVertexConfig.anchorX, y:this.currentVertexConfig.anchorY}
    }
    else if (this.currentEdgeConfig) {
      return {x:this.currentEdgeConfig.anchorX, y:this.currentEdgeConfig.anchorY}
    }
    else {
      return false
    }
  ...
  public drawCall() {
    ...
    this.ctx.clearRect(0, 0, this.canvas.width, this.canvas.height)
    if (this.currentVertexConfig) {
      this.drawGraph(this.ctx as CanvasRenderingContext2D, this.currentVertexConfig)
    }
  ...
  private onDragEnd() {
    if (this.currentVertexConfig) {
      mitt.emit('dragVertexEnd', {x:this.currentVertexConfig.anchorX, y:this.currentVertexConfig.anchorY})
    }
    if (this.currentEdgeConfig) {
      mitt.emit('dragEdgeEnd', {x:this.currentEdgeConfig.anchorX, y:this.currentEdgeConfig.anchorY})
    }
  }
