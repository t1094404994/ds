...
import mitt, {InsertVertex, OperEventData, DrapEventData, EdgeEventData} from './mitt'
import {BgLayer, TopLayer, OperationStatusBit, DrawStatusBit, LayerType, Pt, DrawConfig, updateDrawConfig} from './canvas'
import {createDrawClickConfig, createEdgeConfig} from './canvas'
import FlowChart, {GridGraph, GridConfig, getPT, getGridIndex, updateEdgeLinePath, getGridPt, EdgeData, VertexType} from './graph'
...
export default class Controller {
  ...
  constructor(data:ConsolerData, _gridConfig:GridConfig) {
    ...
    this.data = data
    this.backgroundLayer = new BgLayer(data.backgroundCtx, data.backgroundCanvas)
  ...
  public updateBgConfig() {
    ...
    this.backgroundLayer.setConfigs(drawConfigs, textConfigs, edgeConfigs, edgeTextConfigs)
  }
  ...
  private setOperationStataus(config:DrawConfig, layer:LayerType) {
    ...
    //hover
    config.drawStatus = setBit(config.drawStatus, DrawStatusBit.SHADOW, this.checkHover(config.operationStatus, layer))
    //click
    config.drawStatus = setBit(config.drawStatus, DrawStatusBit.CLICK, this.checkClick(config.operationStatus, layer))
  ...
  private onVertextClick(evt:OperEventData) {
    ...
    if (evt.layer === LayerType.BG && data) {
      ...
      this.backgroundLayer.setClickBox(createDrawClickConfig(data.drawConfig.box))
      this.backgroundLayer.drawCall()
    }
  }
  ...
  private onDragVertexStart(evt:DrapEventData) {
    ...
    data.drawConfig.drawStatus = setBit(drawStatus, DrawStatusBit.DROPSHADOW, true)
  ...
  //找到所有新通路
  private findNewPath(vertex:number, nowGridIndex:number, oldGridIndex:number, edges:Array<EdgeIJ>) {
    ...
    for (let i = 0, l = edges.length; i < l; i++) {
      ...
      const startGridIndex = edgeij.i === vertex ? nowGridIndex : startData.gridIndex
      const endGridIndex = edgeij.j === vertex ? nowGridIndex : endData.gridIndex
  ...
  private updateEdgeLine = (nowGridIndex:number, pathArr:Array<Array<number>>, edgeDataArr:Array<EdgeData>, edges:Array<EdgeIJ>) => {
    pathArr.forEach((path, index) => {
      const edgedata = edgeDataArr[index]
      edgedata.gridPath = path
      const edgeij = edges[index]
      const linePath = this.getLinePath(path, this.gridConfig, edgeij.i, edgeij.j)
  ...
  private updateDropVertexStatus(config:DrawConfig) {
    ...
    config.operationStatus = setBit(operationStatus, OperationStatusBit.DOWN, false)
  }
  ...
  private onDragEdgeStart(evt:DrapEventData) {
    ...
    const config = createEdgeConfig([{x:evt.x, y:evt.y}, {x:evt.x, y:evt.y}])
    if (!config) return
  ...
  private onEdgeClick(data:EdgeEventData) {
    ...
    if (edgeData && edgeData.textConfig) {
      edgeData.textConfig.operationStatus = setBit(edgeData.textConfig.operationStatus, OperationStatusBit.DOWN, false)
    }
  }
  ...
  private showMessage(type:'success' | 'warning' | 'info' | 'error', message:string) {
  ...
  public checkVerter(gridIndex:number) {
    if (this.flowChart.gridExists(gridIndex)) {
      if (gridIndex !== this.flowChart.getVertexData(this.currentVertex)?.gridIndex) this.showMessage('warning', '该网格点已存在')
      return false
    } else {
      return true
    }
  ...
  //插入新的顶点
  public insertVecter(data:InsertVertex, _gridIndex?:number, bpmnData?:VertexBPMNData) {
    //开始和结束节点只能有一个
    ...
    const gridIndex = _gridIndex === undefined ? getGridIndex(data.pt, this.gridConfig) : _gridIndex
    if (!this.checkVerter(gridIndex)) return false
    const vertexData = createVertextData(data.vertexType, gridIndex, this.gridConfig, bpmnData)
  ...
  //根据网格路径获取真实路径
  private getLinePath(gridPath:Array<number>, gridConfig:GridConfig, i:number, j:number) {
  ...
  public insterEdge(i:number, j:number, bpmnData?:EdgeBPMNData) {
    ...
    const edgeData = createEdgeData(linePath, path, i, j, vertexDataIId, vertexDataJId, disable, bpmnData)
    if (!edgeData) {
  ...
  public save(id:string, name:string|false) {
    ...
    if (hasNode === false) {
      ElMessage.warning('缺少中间节点!')
      return false
    }
    if (hasEnd === false) {
      ElMessage.warning('缺少结束节点!')
      return false
    }
    if (!this.checkFlowchartPath(hasStart as number, hasEnd as number)) {
      ElMessage.warning('流程图通路不完整!')
      return false
    }
  ...
  public getCurrentEdgeData = () => {
    ...
    const edgeData = this.flowChart.getEdgeData(this.currentEdge.i, this.currentEdge.j)
    return edgeData
  }
