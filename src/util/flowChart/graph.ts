...
//根据网格坐标获取网格顶点下标
export const getGridIndexByGridPt = (
  pt: Pt,
...
export const getPT = (gridIndex: number, gridConfig: GridConfig): Pt => {
  ...
  x = Math.floor(
    x * (gridConfig.colWidth + gridConfig.colGap) + gridConfig.colWidth / 2
  );
  y = Math.floor(
    y * (gridConfig.rowHeight + gridConfig.rowGap) + gridConfig.rowHeight / 2
  );
  return { x, y };
};
...
//创建顶点数据
export const createVertextData = (
  type: VertexType,
  gridIndex: number,
  gridConfig: GridConfig,
  _data?: VertexBPMNData
): VertexData => {
  ...
  const textConfig = createTextConfig(
    pt,
    text,
    drawConfig.box[1] - drawConfig.box[0]
  );
  let data = _data;
  if (!data) {
...
export const getOffset = (
  ...
  let offsetX =
    secondPt.x > fristpt.x ? wHalf : secondPt.x < fristpt.x ? -wHalf : 0;
  let offsetY =
    secondPt.y > fristpt.y ? hHalf : secondPt.y < fristpt.y ? -hHalf : 0;
  if (contrary) {
...
export class GridGraph extends Graph<boolean, boolean> {
  ...
  private getWeight(startGridIndex: number, endGridIndex: number) {
    ...
    if (startGridIndex !== endGridIndex && this.V[startGridIndex].data)
      return 0;
    else
      return (
        max -
    ...
    for (let i = 0, l = list.length; i < l; i++) {
      ...
      switch (this.V[u].status) {
        ...
        default:
          ...
          edge.status =
            this.V[startV].dTime < this.V[u].dTime
              ? EStatus.FORWARD
              : EStatus.CROSS;
          break;
      }
