...
//计算二元运算符数据
const calculationBinary = (operator:BinaryOpertor, operNum1:number, operNum2:number):number => {
  switch (operator) {
  case '+':
 ...
 */
export const calculationExpression = (_expression:string|undefined):number|undefined => {
  ...
  while (operStack.length) {
    ...
    if (!isNaN(+nowOper)) {
      ...
      case '>':
        ...
        const calcNum = calculationBinary(oper as BinaryOpertor, operNum1, operNum2)
...
//读取字符串的值 公共方法
export const strNumReader = (str:string, startIndex?:number):number|false => {
